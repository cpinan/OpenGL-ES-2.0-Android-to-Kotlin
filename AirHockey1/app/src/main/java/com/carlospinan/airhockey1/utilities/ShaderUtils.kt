...
object ShaderUtils {
    ...
    private fun compileShader(type: Int, shaderCode: String): Int {
        val shareObjectId = OpenGLES20.gl2CreateShader(type)
        if (shareObjectId == 0) {
         ...
         */
        OpenGLES20.gl2ShaderSource(shareObjectId, shaderCode)

        /**
         * This tells OpenGL to compile the source code that was previously uploaded to shaderObjectId.
         */
        OpenGLES20.gl2CompileShader(shareObjectId)
        ...
        OpenGLES20.gl2GetShaderIv(shareObjectId, compileStatus)

        val shaderInfoLog = OpenGLES20.gl2GetShaderInfoLog(shareObjectId)

        log("Results of compiling source:\n$shaderCode\n:$shaderInfoLog")

        if (compileStatus[0] == 0) {

            OpenGLES20.gl2DeleteShader(shareObjectId)
    ...
    fun linkProgram(vertexShaderId: Int, fragmentShaderId: Int): Int {
        ...
        OpenGLES20.gl2AttachShader(programObjectId, vertexShaderId)
        OpenGLES20.gl2AttachShader(programObjectId, fragmentShaderId)

        OpenGLES20.gl2LinkProgram(programObjectId)

        val shaderInfoLog = OpenGLES20.gl2GetShaderInfoLog(programObjectId)

        val linkStatus = IntArray(1)
        OpenGLES20.gl2GetProgramiv(programObjectId, GL_LINK_STATUS, linkStatus)

        log("Results of linking program:\n:$shaderInfoLog")

        if (linkStatus[0] == 0) {
            OpenGLES20.gl2DeleteProgram(programObjectId)
    ...
    fun validateProgram(programObjectId: Int): Boolean {
        ...
        OpenGLES20.gl2GetProgramiv(programObjectId, GL_VALIDATE_STATUS, validateStatus)
